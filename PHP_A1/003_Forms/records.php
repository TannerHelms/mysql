...
if ($stmt = $mysqli->prepare("UPDATE players SET firstname = ?, lastname = ?, email = ?
WHERE id=?"))
{
$stmt->bind_param("ssi", $firstname, $lastname,email, $id);
$stmt->execute();
$stmt->close();
}
...
// show the form
renderForm($firstname, $lastname, $email, $id);
...
if ($stmt = $mysqli->prepare("INSERT players (firstname, lastname, email) VALUES (?, ?)"))
{
